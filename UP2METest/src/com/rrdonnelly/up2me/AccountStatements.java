...
/**
 * Created by rahul.k on 12/4/13.
 */
public class AccountStatements extends Activity {
    ...
    List<Statement> statmentsListBuild;
    public List<Statement> filteredstatmentsList;
   // public List<Statement> statmentsListdata;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        sortBy = " order by substr(periodEndDate , 7, 4)|| '-' || substr(periodEndDate , 1, 2) || '-' || substr(periodEndDate , 4, 2)  desc,providerName asc";
        ...
        if (data != null){
	      ...
	      //  statmentsListdata = new StatementDAO().getAllAccountStatements(this, loginUserId," order by substr(periodEndDate , 7, 4)|| '-' || substr(periodEndDate , 1, 2) || '-' || substr(periodEndDate , 4, 2)  desc,providerName asc");	        
            ...
            statmentsListBuild = statementDAO.getAllAccountStatements(this, loginUserId," order by substr(periodEndDate , 7, 4)|| '-' || substr(periodEndDate , 1, 2) || '-' || substr(periodEndDate , 4, 2)  desc,providerName asc");
           // statmentsListdata = statmentsListBuild;
            //displayStatment(statmentsListBuild);
        }
    ...
    public void sortFunction( int id,String title){
    	switch (id) {
	      case R.id.receivedDateMenu:
	    	  sortBy = " order by substr(periodEndDate , 7, 4) || '-' || substr(periodEndDate , 1, 2) || '-' || substr(periodEndDate , 4, 2)  desc,providerName asc";
	    	  statmentsListBuild = new StatementDAO().getAllAccountStatements(currentActivity,loginUserId," order by substr(periodEndDate , 7, 4) || '-' || substr(periodEndDate , 1, 2) || '-' || substr(periodEndDate , 4, 2)  desc,providerName asc");
	    	  displayStatment(statmentsListBuild);
	    	  txtView.setText(title);
	    	  selectItemID= R.id.receivedDateMenu;
	        	break;
	      case R.id.dueDateMenu:
	    	  sortBy = " order by substr(periodEndDate , 7, 4)|| '-' || substr(periodEndDate , 1, 2) || '-' || substr(periodEndDate , 4, 2)  desc";
	    	  statmentsListBuild = new StatementDAO().getAllAccountStatements(currentActivity,loginUserId," order by substr(periodEndDate , 7, 4)|| '-' || substr(periodEndDate , 1, 2) || '-' || substr(periodEndDate , 4, 2)  desc");
	    	  displayStatment(statmentsListBuild);
	    	  txtView.setText(title);
	    	  selectItemID= R.id.dueDateMenu;
	    	  break;
	      case R.id.byProviderMenu:
	    	  sortBy = " order by providerName asc,substr(periodEndDate , 7, 4) || '-' || substr(periodEndDate , 1, 2) || '-' || substr(periodEndDate , 4, 2)  desc ";
	    	  statmentsListBuild = new StatementDAO().getAllAccountStatements(currentActivity,loginUserId," order by providerName asc,substr(periodEndDate , 7, 4) || '-' || substr(periodEndDate , 1, 2) || '-' || substr(periodEndDate , 4, 2)  desc ");
	    	  displayStatment(statmentsListBuild);
	    	  txtView.setText(title);
...
//					List<Statement> statementList = statementsMap.get("Statement");
    ...
    public void showDialog(){
    	...
    	if (statement.isRead()){
    		alertDialog2.setMessage("Statement content will be marked as unread");
    	}else{
    		alertDialog2.setMessage("Statement content will be marked as read");
    	}
    	alertDialog2.setPositiveButton("Ok",
    	        new DialogInterface.OnClickListener() {
    	            public void onClick(DialogInterface dialog, int which) {
    	            	...
    	            	List<Statement> statementsList = new StatementDAO().getAllAccountStatements(currentActivity, loginUserId,"");
    ...
    public void showPopup(){
	    ...
	    PopupMenu popup = new PopupMenu(AccountStatements.this, txtView);
	    //Inflating the Popup using xml file
	    popup.getMenuInflater().inflate(R.menu.account_statements, popup.getMenu());
...
//	        Intent assistantsIntent = new Intent(view.getContext(), AssistantsActivity.class);
//	        assistantsIntent.putExtra("usertoken", userToken);
//	        assistantsIntent.putExtra("userName", userName);
//	        assistantsIntent.putExtra("salt", salt);
//	        assistantsIntent.putExtra("loginUserId", loginUserId);
//	        startActivityForResult(assistantsIntent, 0);
//    	}
    }
	    ...
	    protected void onResume() {
	    	...
	    	//displayStatment(statmentsListBuild);
	    	 ...
	    	 if(filterText != "" && filterText.length() > 0) {
		        	...
		        	statmentsListBuild = new StatementDAO().getAllAccountStatements(currentActivity, loginUserId,sortBy);
		        	displayStatment(statmentsListBuild);
		        }
	             ...
	             public boolean onQueryTextChange(String newText) {
	 				...
	 				if (cm != null && cm.getActiveNetworkInfo() != null	&& cm.getActiveNetworkInfo().isConnected()) {
	 					if("".equals(newText)){
	 						...
	 						displayStatment(statmentsListBuild); //statmentsListdata
	 					}else {
	 						getMetaDataSearch(currentActivity, loginUserId, newText.toString().toUpperCase());
	 					}
	 				}else{
	 	            	List<Statement> finalSearchedList = new ArrayList<Statement>();
	 	            	if(statmentsListBuild.size() > 0) { //statmentsListdata
	 	            		for (int i = 0; i < statmentsListBuild.size(); i++) { //statmentsListdata
	 	            		    Statement itemStatement = statmentsListBuild.get(i); //statmentsListdata
